...
export function useCollectionAction() {
  ...
  const uncollectMutation = useMutation({
    mutationFn: async ({
      ...
      const statusRes = type === 'answer' 
        ? await getAnswerCollectionStatus(id) 
        : await getArticleCollectionStatus(id);
      
      const favoritedFolders = statusRes?.data?.filter((item: any) => item.is_favorited) || [];
  ...
  const handleCollect = (id: string | number, type: 'answer' | 'article', isCurrentlyCollected: boolean) => {
    if (!cookies) {
  ...
  return {
    collect: (id: string | number, type: 'answer' | 'article') => collectMutation.mutate({ id, type }),
    uncollect: (id: string | number, type: 'answer' | 'article') => uncollectMutation.mutate({ id, type }),
